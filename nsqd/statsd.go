	...
	for {
		...
		case <-ticker.C:
			...
			for _, topic := range stats.Topics {
				...
				_ = client.Gauge(stat, topic.Depth)

				stat = fmt.Sprintf("topic.%s.backend_depth", topic.TopicName)
				_ = client.Gauge(stat, topic.BackendDepth)
				...
				for _, item := range topic.E2eProcessingLatency.Percentiles {
					...
					// accuracy
					_ = client.Gauge(stat, int64(item["value"]))
				}
				...
				for _, channel := range topic.Channels {
					...
					_ = client.Gauge(stat, int64(channel.ClientCount))
			...
			if n.getOpts().StatsdMemStats {
				...
				_ = client.Gauge("mem.heap_objects", int64(ms.HeapObjects))
				_ = client.Gauge("mem.heap_idle_bytes", int64(ms.HeapIdleBytes))
				_ = client.Gauge("mem.heap_in_use_bytes", int64(ms.HeapInUseBytes))
				_ = client.Gauge("mem.heap_released_bytes", int64(ms.HeapReleasedBytes))
				_ = client.Gauge("mem.gc_pause_usec_100", int64(ms.GCPauseUsec100))
				_ = client.Gauge("mem.gc_pause_usec_99", int64(ms.GCPauseUsec99))
				_ = client.Gauge("mem.gc_pause_usec_95", int64(ms.GCPauseUsec95))
				_ = client.Gauge("mem.next_gc_bytes", int64(ms.NextGCBytes))
				_ = client.Incr("mem.gc_runs", int64(ms.GCTotalRuns-lastMemStats.GCTotalRuns))
